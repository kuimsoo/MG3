            ...
            <div class="category_product_wrap">
                <div>
                 	<c:forEach items="${arItem}" var="item">
        				<div>
            				<strong class="category_title">${item.item_name}</strong><input type='text' id='hiddenid'><input type='text' id='userid' value='${sessionScope.userid}'>
...
.ready(function() {
 ...
 $('#hiddenid').val(id);
	...
	$.ajax({
		url:'/countcart',type:'post',data:{},dataType:'text',
...
.on('click','#btncart',function(){
		...
		let qty=$('#cnt').val();		
		let totalpriceStr = $('#totalprice').val(); // 예: "66,000"
		// 2. 쉼표를 제거하고 숫자로 변환합니다
		let totalprice = parseInt(totalpriceStr.replace(/,/g, ''), 10);
		...
		$.ajax({
			url:'/insertcart',type:'post',data:{item_id:item_id,qty:qty,totalprice:totalprice},dataType:'text',
			success:function(data){
			if(data=='ok'){
